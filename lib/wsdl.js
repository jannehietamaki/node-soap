...
var WSDL = function(definition, uri, options) {
    ...
    this._serializer = new Serializer(this, options);
...
var Serializer = function (wsdl, options) {
    ...
    this.element = function(element, obj, alias, schema) {
        ...
        var attr = "";
        var newNamespace = options.strictNamespaces === true ? false : true;
        ...
        // Adds start element
        var elemName = (newNamespace && ns.alias ? ns.alias + ":"  : "") + element["$name"];
        if(obj == undefined && element['$nillable'] == 'true') {
            // Ok, no need to output undefined nillable object
        } else {
    ...
    this.sequence = function(element, obj, alias, schema) {
        ...
        if (obj instanceof Array) {
            ...
            assert(!obj || (obj && typeof obj === "object"), "Unknown sequence type.");
            element.children.forEach(function(child) {

                var name = child["$name"];
                var data;
                if (name) {
                    data = obj ? obj[name] : undefined;
                } else {
                    data = obj;
                }
                if ((data===null || data===undefined)  && child["$nillable"] !== 'true' && child["$minOccurs"] !== '0') {
